...
render: -> """
  <div class='top'>
    ...
    <table class='raspData' id='systemSection'>
      ...
      <tr>
        <td class='description'>Wifi Address</td>
        <td class='value'><span id='wifiAddress'>N/A</span></td>
      </tr>
      <tr>
...
update: (output,domEl) ->
  ...
  if values[0].includes("host not found")
    ...
    div.find('#wifiAddress').html(values[7])
    div.find('#kernelVersion').html("Linux " + values[8])

    # Split disk usage output
    diskvalues	= values[9].split(" ")
    usedDiskSpace = diskvalues[1]-diskvalues[3];
    diskSpace =  usedDiskSpace+ " of " + diskvalues[1]+ " GB Used &nbsp;"
    div.find('#diskUsage').html( diskSpace)
    div.find('#barFillingDisk').width((diskvalues[1]-diskvalues[3])/diskvalues[1]*100*2.5)

    div.find('#runningProcs').html(values[10])
    div.find('#cpuType').html(values[11])
    div.find('#cpuRevision').html(values[12])
    div.find('#cpuModel').html(values[13])
